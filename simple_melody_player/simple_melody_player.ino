...
// Change this value to adjust the time of the song
int time = 140;

// Pin connected to the buzzer
int buzzer = 9;

// Melody array: note and duration (positive for normal, negative for dotted)
int melody[] = {
  ...
  NOTE_G4, 4, NOTE_E4, -2, NOTE_B4, -4, NOTE_E5, -4, NOTE_B4, -4, NOTE_E5, -4,
  NOTE_B4, 8,  NOTE_E5, -4, NOTE_B4, 8, REST, 8, NOTE_AS4, 8, NOTE_B4, 8,
  ...
  NOTE_G4, -4, NOTE_E4, -2, NOTE_E4, 2, NOTE_G4, 2,
  NOTE_B4, 2, NOTE_D5, 2, NOTE_F5, -4, NOTE_E5, -4, NOTE_AS4, 8, NOTE_AS4, 8, NOTE_B4, 4, NOTE_G4, 4
};
...
void loop() {
  ...
  for (int i = 0; i < notes * 2; i += 2) {
    ...
    int noteDuration = (wholenote) / abs(duration);

    // Adjust for dotted notes
    if (duration < 0) noteDuration *= 1.5;

    // Play the note if it's not REST
    if (note) {
      tone(buzzer, note, noteDuration * 0.9); // Play note for 90% of the duration
  ...
  delay(5000); // Wait for 5 seconds before repeating the melody
}